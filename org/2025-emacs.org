...
一方、 =consult-find-file= はサブディレクトリを含めた再帰的な検索を行い、 CWD 的の変更に相当する操作がありません。
...
#+BEGIN_QUOTE
Wiki 版と異なり Enter キーでファイルを開ける他、従来の =find-file= と同様に CWD を含めて入力欄に表示しています。
#+END_QUOTE
...
VSCode のサイドバーは検索結果を保持できて良さそうですが、 minibuffer も開きっぱなしにできるようになりました。もちろん =embark-export= で退避させても良さそうです。
